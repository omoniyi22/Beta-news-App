...
class SinglePost extends React.Component {
	...
	componentWillReceiveProps(nextP) {
		if (nextP.comment) {
			this.state.posts[0].comments.push(nextP.comment)
		}
	}
	check() {
		this.setState((prev) => {
			...
			<>
				{!this.state.loading ?
					...
					(this.state.posts.map(post => (
						post !== undefined && (
							<div className='singlePost px-0 col-md-8 px-3 mx-0 mt-0 pt-1'>
								<div className='Post row'>
									...
									<img src={post.picture} className=' border text-center mx-auto POST_IMG mb-sm-3 mb-md-4' alt='POST_IMG' />
									<div className='content ssk col-12 px-sm-2 px-md-3 mt-sm-3 mt-md-3 py-md-1 '>
								...
								<div className='step-content  mr-xl-0 p-0 mt-2 container-fluid '>
									<div className="row  w-100 px-0 mx-auto">
										{post.comments.map(pos => <NewsTwo1c user={pos.user} comment={pos.value} date={pos.date} />)}

									</div>
								</div>
								<form class="form-horizontal row ssk mt-3" noValidate onSubmit={(e) => {
									e.preventDefault()
									if (`${this.state.comment}`.length > 1) {
										let { posts, comment, name } = this.state
										let men = [posts[0].type, posts[0]._id, `${name}`, `${comment}`]
										...
										<div class="form-group row">
											<div class="offset-sm-2 col-sm-10">
												<div class="form-check">
													...
													<label class="form-check-label" for="exampleCheck2">Remain Annonymous</label>
												</div>
											</div>
										</div>
										<div class="form-group row">
									...
									<span className="w-100 text-right">
										<button type="submit" class="btn btn-primary float-right btn-sm px-2 mt-0 mr-3 rounded-lg">SEND <span className="ml-1 fa fa-paper-plane" /></button>
									</span>
								</form>
								<NewTwo1b />
...
export default connect(mapStateToProps, { Comment })(SinglePost);
