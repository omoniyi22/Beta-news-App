	...
	render() {
		return (
			this.props.posts.map(post => (
				post !== undefined && (<div style={{ backgroundImage: `url(${post.picture})`, backgroundSize: '100% 150%', backgroundRepeat: 'no-repeat', backgroundOrigin: 'contentBox' }} className='col-md-6 px-0 pt-0 newsOne2 border '>
					...
					<div className='newsIconNews py-1 px-0 px-2 pb-2'>
						<Link to={`/post/${post.type + '+' + post._id}`} className="text-white">
							...
							<div className='newsItems  p-0  mb-2 mt-1 text-capitalize'>News - {post.type}</div>
