...
const entertainmentDB = require('../../../models/News/entertainment')
const error = []


//Posting a single post
router.post('/entertainment', (req, resp) => {
    const {
        title,
        description,
        picture,
    ...
    if (error.length > 0) {
        ...
        entertainmentDB.find().then(res => {
            const newPost = new entertainmentDB({
...
// Update a SinglePost
router.post('/update/entertainment/:id', (req, res) => {
...
//Delete any Politics
router.get('/delete/entertainment/:id', (req, res) => {
    entertainmentDB.findById(req.params.id).then(() => {
        const Delete = async (d = req.params.id) => {
            try {
                await entertainmentDB.findByIdAndDelete(d)
...
router.post('/views/entertainment/:id', (req, res) => {
...
router.post('/delete/views/entertainment/:id', (req, res) => {
