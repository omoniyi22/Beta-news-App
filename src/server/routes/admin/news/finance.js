...
const financeDB = require('../../../models/News/finance')
const error = []


//Posting a single post
router.post('/finance', (req, resp) => {
    const {
        title,
        description,
        picture,
    ...
    if (error.length > 0) {
        ...
        financeDB.find().then(res => {
            const newPost = new financeDB({
...
// Update a SinglePost
router.post('/update/finance/:id', (req, res) => {
...
//Delete any Politics
router.get('/delete/finance/:id', (req, res) => {
    financeDB.findById(req.params.id).then(() => {
        const Delete = async (d = req.params.id) => {
            try {
                await financeDB.findByIdAndDelete(d)
...
//Deleting a Comment
router.post('/comment/finance/:id', (req, resp) => {
    financeDB.findById(req.params.id).then((member) => {
...
router.post('/views/finance/:id', (req, res) => {
...
router.post('/delete/views/finance/:id', (req, res) => {
