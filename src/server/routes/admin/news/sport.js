...
const sportDB = require('../../../models/News/sport')
const error = []


//Posting a single post
router.post('/sport', (req, resp) => {
    const {
        title,
        description,
        picture,
    ...
    if (error.length > 0) {
        ...
        sportDB.find().then(res => {
            const newPost = new sportDB({
...
// Update a SinglePost
router.post('/update/sport/:id', (req, res) => {
...
//Delete any Politics
router.get('/delete/sport/:id', (req, res) => {
    sportDB.findById(req.params.id).then(() => {
        const Delete = async (d = req.params.id) => {
            try {
                await sportDB.findByIdAndDelete(d)
...
//Deleting a Comment
router.post('/comment/sport/:id', (req, resp) => {
    sportDB.findById(req.params.id).then((member) => {
...
router.post('/views/sport/:id', (req, res) => {
...
router.post('/delete/views/sport/:id', (req, res) => {
